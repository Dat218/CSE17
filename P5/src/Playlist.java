		...
		if (args.length > 0) {
			...
			for (int m=0; m<playlists.size(); m++) {
				if (playlists.get(m) != null) {	
					...
					else if (m == playlists.size()-1 && count != 1) {
			...
			for (int n=0; n<playlists.size(); n++) {
				if (playlists.get(n) != null) {
					System.out.printf("First and last song in the %s\n", playlists.get(n).getName());
					System.out.println(playlists.get(n).getSong(1));
					System.out.println(playlists.get(n).getSong(playlists.get(n).getNumSongs()));
					break;
				}
				else 
					continue;
